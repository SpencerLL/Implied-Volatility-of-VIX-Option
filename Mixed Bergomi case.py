...
class HermiteApproximation:
    ...
    def func_g1(self, y):
        ...
        clipped_arg_exp_g = np.clip(arg_exp_g, EXP_LOWER_BOUND, EXP_UPPER_BOUND)
        exp_val = np.exp(clipped_arg_exp_g)
        g_sq = 1.0 + self.b * exp_val
        safe_g_sq = np.maximum(0, g_sq)

        g1_val = np.where(
            safe_g_sq > 1e-10,
            0.5 * self.b * exp_val / np.sqrt(safe_g_sq),
            0.0
        )
    ...
    def weight_calculation(self, n, func_g_):
        ...
        try:
            integral1, err1 = quad(integrand, -np.inf, 0, limit=2000, epsabs=EPSILON, epsrel=EPSILON)
            integral2, err2 = quad(integrand, 0, np.inf, limit=2000, epsabs=EPSILON, epsrel=EPSILON)
    ...
    def implied_vol_bell_polynomial(self, best_N, type):
            ...
            if type == 1:
                ...
                if type == 2:
                    ...
                    if (B ** 2 - 2 * (x0 - k)) >= 0:
                        ...
                        raise ValueError("The discriminant is less than 0, this method is not applicable.")
            k_2 = x0 + B * self.sigma1 / 2 + self.var1 / 8
        else:
            B = A - self.sigma2 / 2
            if type == 1:
                iv0 = 0.5 * self.sigma2 / np.sqrt(self.T)
                ...
                if type == 2:
                    ...
                    if (B ** 2 - 2 * (x0 - k)) >= 0:
                        ...
                        raise ValueError("The discriminant is less than 0, this method is not applicable.")
            k_2 = x0 + B * self.sigma2 / 2 + self.var2 / 8
        ...
        if type == 1:
            ...
            extra_term = (np.exp(k_2) - self.kappa) * norm.cdf(-A)
        else:
            extra_term = self.kappa * (norm.cdf(-B - iv0 * np.sqrt(self.T)) - norm.cdf(-A))
    ...
    def implied_vol_bell_polynomial_e(self, best_N, type):
        ...
        iv1 = psi[0] / (f * np.sqrt(self.T))
        iv2 = (psi[1] * (iv0 * np.sqrt(self.T)) ** 2) / f * (- 0.5 * 1 / (iv0 * self.T) - m / (iv0 ** 3 * self.T ** 2)) - 0.5 * iv1 ** 2 * m1
        iv3 = (psi[2] * (iv0 * np.sqrt(self.T)) ** 3) / f * (0.25 * 1 / (iv0 * self.T) - (m - 1) / (iv0 ** 3 * self.T ** 2) + m ** 2 / (iv0 ** 5 * self.T ** 3)) - \
              iv1 * iv2 * m1 - 1 / 6 * iv1 ** 3 * m2

        iv1 = psi[0] / (f * np.sqrt(self.T))
        iv2 = (psi[1] * (iv0 * np.sqrt(self.T)) ** 2) / f * (- 0.5 * 1 / (iv0 * self.T) - m / (iv0 ** 3 * self.T ** 2))
        iv3 = (psi[2] * (iv0 * np.sqrt(self.T)) ** 3) / f * (0.25 * 1 / (iv0 * self.T) - (m - 1) / (iv0 ** 3 * self.T ** 2) + m ** 2 / (iv0 ** 5 * self.T ** 3))
...
class optimal_N:
    ...
    def __init__(self, obj, N_max, M, T):
        ...
        self.y_eval = np.linspace(-10, 10, M, dtype=float)
        self.y_plot = np.linspace(-10, 10, M, dtype=float)

        self.g_y = obj.func_g(self.y_eval)
        self.g_y_plot = obj.func_g(self.y_plot)

        self.phi = norm.pdf(self.y_eval)
    ...
    #             F_errors[N] = np.nan
    #         else:
    #             squared_errors = np.abs(gN_approx * self.phi - self.g_y * self.phi) ** 2
    #             F_errors[N] = np.sum(squared_errors)
    #             # errors = np.abs(gN_approx - self.g_y)
    #             # F_errors[N] = np.maximum(errors)
    #
    #     best_N = 0
...
def main_mb():
    ...
    for t in T:
        ...
        # iv_approx[j] = np.array([HermiteApproximation(kappa, mb, p=1).implied_vol_bell_polynomial(best_N, 2)[0] for kappa in kappa_list])
