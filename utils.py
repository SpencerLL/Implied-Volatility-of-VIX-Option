...
def plot_style_4(kappa, maturities, reference, approx, expansion, x_title, y_title, title):
    """
    ...
    err_approx = {m: signed_rel_error(reference[m], approx[m]) for m in maturities}
    err_expan = {m: signed_rel_error(reference[m], expansion[m]) for m in maturities}

    fig, axs = plt.subplots(2, 2, figsize=(16, 16))
    
    for i, m in enumerate(maturities.keys()):

        c = colors[i % len(colors)]

        axs[0, 0].plot(kappa, reference[m], "x", color=c, label=f"ref({m})")
        axs[0, 0].plot(kappa, approx[m], "--", color=c, label=f"approx({m})")
        
        axs[0, 1].plot(kappa, reference[m], "x", color=c, label=f"ref({m})")
        axs[0, 1].plot(kappa, expansion[m], "--", color=c, label=f"expan({m})")
        
        axs[1, 0].plot(kappa, err_approx[m], "+--", color=c, label=m)
        axs[1, 1].plot(kappa, err_expan[m], "+--", color=c, label=m)

    ylabels_list = [y_title, y_title, "Relative error (%)", "Relative error (%)"]
    
    for ax, y_lab in zip(axs.flat, ylabels_list):
        ax.set_xlabel(x_title)
        ax.set_ylabel(y_lab)

    axs[0, 0].legend(ncol=3, frameon=True, fontsize='small')
    axs[0, 1].legend(ncol=3, frameon=True, fontsize='small')
    axs[1, 0].legend(ncol=1, frameon=True, fontsize='small')
    axs[1, 1].legend(ncol=1, frameon=True, fontsize='small')
